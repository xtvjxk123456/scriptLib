...
class MDLInfo(CharInfo):
    def __init__(self, refnode):
        super(MDLInfo, self).__init__(refnode)

    def _get_versions(self):
        MDL_publish = r'Z:/Shotgun/projects/df/_library/assets/Environment/{}/mdl/_publish/'.format(self.asset)
        versions = ps.find_folders_in_dir(os.path.normpath(MDL_publish))
    ...
    def update_file(self):
        ...
        file_base_name = 'df_' + self.asset + '_mdl_' + current_version

        current_publish = r'Z:/Shotgun/projects/df/_library/assets/Environment/{}/mdl/_publish/{}/{}_*'.format(
            self.asset,
            current_version,
            file_base_name)
        ...
        if len(file_paths) == 1:
            ...
            pm.warning(u'{}的mdl publish目录有问题'.format(self.asset))


class MainUI(qg.QWidget):
    def __init__(self):
        super(MainUI, self).__init__()
        self.setWindowTitle("Only change Environment MDL version in Anim")
        ...
        self.tilte = qg.QLabel('just only change Environment mdl version(rig is not working)')
